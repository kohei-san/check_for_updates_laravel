      ...
      @foreach($customers as $customer)
        <tbody>
          <tr>
            ...
            {{-- ▲JS編集用customer_id --}}
            {{-- ▼差分の表示 --}}
            @if($customer->long_diff != null)
              @if($customer->long_diff->difference_flg == 1)
              <x-table-td :active="$count % 2 == 1">
            ...
            {{-- 差分がある場合は時間表示 --}}
            @if($customer->long_diff->time_stamp_dif_long != '0000-00-00 00:00:00' || $customer->long_diff->time_stamp_dif_long != '0000-00-00 00:00:00')
              <x-table-td :active="$count % 2 == 1">{{ __(\Carbon\Carbon::parse($customer->long_diff->time_stamp_dif_long)->diffForHumans()) }}</x-table-td>
            @else
